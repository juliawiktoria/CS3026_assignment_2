...
void mymkdir ( const char * path )
{
   ...
   while ((token = strtok_r(rest, "/" , &rest)))
   {
      ...
      diskblock_t block = virtualDisk[3];
      ...
      // find next empty directory and get its index
      int dirIndex = emptyDirIndex(block);
      
      //set filename to block
      strcpy(block.dir.entrylist[dirIndex].name, token);
      
      // mark block as in use
      block.dir.entrylist[dirIndex].unused = FALSE;
      ...
      block.dir.entrylist[dirIndex].firstblock = freeFATindex;
      ...
      blocknumber = freeFATindex;
   }
}

char ** mylistdir (const char * path)
{
   // create an array of pointers for the result
